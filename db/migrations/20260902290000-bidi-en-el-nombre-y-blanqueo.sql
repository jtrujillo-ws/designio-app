...
-- Repara ANTES de restringir, igual que 20260902150000 con las extensiones: una migración
-- forward-only tiene que poder correr sobre una base con datos. Y no en silencio.
...
set nombre = coalesce(
      nullif(btrim(regexp_replace(a.nombre, '[\u200e\u200f\u202a-\u202e\u2066-\u2069]', '', 'g')), ''),
      'adjunto')
where not sin_overrides_bidi(a.nombre);
