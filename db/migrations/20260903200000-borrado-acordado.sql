...
--    así que no toman el candado del workspace. Cualquier escritura sobre esas dos que haya
--    pasado su comprobación de política ANTES de que este guard vaciara puede commitear
--    DESPUÉS, y el recuento no la incluye porque no la vio.
--
--    Y no es solo una exportación en vuelo, que es como se describió primero y se quedaba
--    corto: el rol de aplicación tiene INSERT directo sobre `evento_dominio` —lo usan veinte
--    servicios para auditar—, así que un miembro con SQL crudo puede abrir transacción,
--    escribir un evento con el `payload` que quiera, esperar a que el borrado termine y
--    commitear. Queda contenido elegido por él en un workspace cuya constancia dice que la
--    auditoría se destruyó. No puede leerlo después —la RLS de `evento_dominio` pide rol de
--    miembro y el borrado destruyó su membresía—, ni recuperar nada, ni evitar que los
--    conteos digan la verdad sobre lo que SÍ se destruyó; lo que consigue es dejar FILAS
--    en esas dos tablas, y no necesariamente una: una exportación en vuelo confirma su
--    evento y su fila de registro —las dos las escribe `registrar_exportacion` en la misma
--    transacción—, y por SQL crudo una transacción puede insertar varios eventos antes de
--    commitear. El tamaño de la ventana es «filas en esas dos tablas», no «una».
--
--    ── Por qué no se cierra aquí, con las dos vías medidas ──
--    (1) Serializar esas escrituras con el borrado es lo primero que se probó: el guard que
--    toma el candado convierte a esas tablas en tablas que serializan y releen, y la
--    derivación de 20260902330000 les impone READ COMMITTED — con lo que la exportación, que
--    corre en REPEATABLE READ a propósito para que su manifiesto salga de una sola foto
--    (SYS-04), deja de funcionar. Medido: 18 casos en rojo. Y no vale tomar el candado por
--    fuera de un trigger para esquivar la derivación: eso deja a la exportación esperando el
--    candado y releyendo después una foto anterior, que es EXACTAMENTE el defecto que la
--    derivación existe para impedir, y encima sin el guard que lo atrapa.
--    (2) Quitarle al rol de aplicación el INSERT directo y encauzar toda la auditoría por una
--    función es la solución de fondo, y toca veinte servicios: su propio slice.
--    Mientras tanto la ventana se DICE, aquí y dentro del sello, en vez de fingir que no está.
