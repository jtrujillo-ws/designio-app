...
describeAuthz('árbol de navegación (proyección + aislamiento)', () => {
  ...
  afterAll(async () => {
    const admin = sqlAdmin();
    await admin`delete from reto_servicio_afectado where workspace_id in (${wsA}, ${wsB})`;
    await admin`delete from proyecto where workspace_id in (${wsA}, ${wsB})`;
    await admin`delete from reto where workspace_id in (${wsA}, ${wsB})`;
    await admin`delete from servicio where workspace_id in (${wsA}, ${wsB})`;
  ...
  it('el rol de aplicación no puede escribir el árbol (sin grant hasta que lleguen sus funciones)', async () => {
    await expect(
