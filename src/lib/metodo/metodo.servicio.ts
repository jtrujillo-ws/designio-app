...
export async function aprobarGate(
  ...
  return conUsuario(actorId, async (tx) => {
    ...
    try {
      ...
      // las que no: el traductor cubre la que se añada mañana. Dos códigos, una razón:
      // `DR001` es el de derechos (lo distingue de «rol insuficiente» para que el motivo
      // con la dimensión que falta llegue entero — SYS-14) y `P0001` el genérico de
      // `raise`.
      const err = e as { code?: string; message?: string };
